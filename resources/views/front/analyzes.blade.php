...
@section('content')
    <div class="page-content analyzes">
        <div class="page-title-box">
            <h1>{{ __('main.analyzes.title') }}</h1>
        </div>
        <div class="container">
            <div class="row">
                <div class="col-8 offset-2">
                    <div data-tab-content="1" class="tab-content-box active">
                        <h3>{{ \App\Models\Setting::getDescription('analyzes_description') }}</h3>
                        @if($analyzes->count())
                            @foreach($analyzes as $analyze)
                                <a href="{{ $analyze->attachment->urls['original'] }}" target="_blank">
                                    <div class="analyzes-box">
                                        ...
                                        <h3 style="">{{ translate($analyze, 'title') }}</h3>
                                    </div>
                                </a>
                            @endforeach
                        @else
